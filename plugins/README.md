...
**기대 결과** — 실제 opencode 클라이언트로 검증한 응답의 구조다.

1. 기상특보와 실황 강우가 기관·기준시각·라이선스와 함께 나온다
2. **"확인하지 못한 것"** 항목에 산림청 산사태 API 2종이 403이며 개발단계 심의
   대기 상태라는 사실이 나온다
3. 산사태 위험이 없다고 말하지 않는다 — 조회하지 못했을 뿐이라고 구별한다
4. 대피 여부·우선순위는 담당 공무원의 판단이라고 밝힌다
5. `complete: false`를 명시한다

2번이나 3번이 빠지면 Skill이 적용되지 않은 것이다. MCP 서버만 붙이면 도구는
동작하지만 AI가 빈 결과를 '위험 없음'으로 답할 수 있다.

## 설정 검증

`plugins/opencode.json`은 opencode 공식 스키마(`McpLocalConfig`)로 검증했다.
`type: local`, `command` 배열, `cwd`, `environment`, `enabled`가 모두 스펙에
있는 필드다.

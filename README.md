...
**출처 없이 값이 이동하지 않는다.** 모든 데이터가 `Record`(값 + 출처 + 신선도 + 품질플래그)로 감싸여 다닌다. 값만 꺼내 쓰는 것은 가능하지만, 그러려면 인용을 의도적으로 버려야 한다.

**실패를 숨기지 않는다.** 응답은 `records`와 함께 `degradations`를 싣는다. `complete: false`는 일부 원천 조회에 실패했다는 뜻이고, 결과가 비어 있어도 '해당 없음'이 아니다.

...
실패만 기록하면 "조회하지 않은 원천"과 "조회해서 비어 있던 원천"을 구별할 수 없다.
그래서 조회마다 영수증을 남긴다.

```python
SourceReceipt
├── connector, dataset_id
├── outcome: RECORDS | CONFIRMED_EMPTY | FAILED
├── record_count
├── checked_at, upstream_status
└── detail
```

`CONFIRMED_EMPTY`는 **원천이 정상 응답으로 "해당 없음"을 명시한 경우에만** 쓴다.
응답 구조를 알아보지 못했거나 오류 코드가 왔으면 `FAILED`다. 이 구별이 없으면
파싱 실패가 '위험 없음'이 된다.

`Answer.absence_is_confirmed`가 호출자의 실제 질문에 답한다 — "이 빈 결과를
'해당 없음'으로 읽어도 되는가". 조회한 원천이 하나도 없으면 확인된 것이 없으므로
`False`다.
...
| 상태 | 기준 | 판단 근거로 사용 |
| --- | --- | --- |
| `fresh` | 갱신주기 2배 이내 | 가능 (판단 허용 나이 이내라면) |
| `aging` | 2~6배 | **불가** — 시점을 함께 밝혀야 한다 |
| `stale` | 6배 초과 | 불가 |
| `unknown` | 관측시각 없음, 또는 미래 시각 | 불가 |

**갱신주기와 판단 허용 나이는 다른 개념이다.** 갱신주기는 "얼마나 자주 나오는가"이고,
`max_decision_age_seconds`는 "얼마나 오래된 것까지 대피 판단에 쓸 수 있는가"다.
시간당 갱신 기상자료가 `aging`이면 최대 6시간 전 값인데, 6시간 전 강우를 현재로
제시하는 것은 위험하므로 `aging`은 판단 근거로 쓰지 않는다.

관측 시각이 미래인 경우는 시계 오차 범위(10분)만 허용한다. 그 이상이면 시각 표기
오류이며 `unknown`이다 — 0으로 깎으면 2099년 관측이 영원히 `fresh`로 남는다.
